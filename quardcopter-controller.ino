...
//
#define BATTERY_MAX 1240
#define BATTERY_MIN 800
...
void loop() {
  if (status & B00000001) {
    PORTB |= B00010000;  // Turn on pin12
  } else {
    PORTB &= B11101111;  // Turn off pin12
  }
...
bool isStarted() {
  ...
  if (status == STARTED
      && pulse_length[mode_mapping[YAW]] >= 1940
...
bool isBatteryConnected() {
  ...
  return battery_voltage < BATTERY_MAX && battery_voltage > BATTERY_MIN;
}
